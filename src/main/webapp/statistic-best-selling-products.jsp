...
<body>
	...
	<%
	// tạo list số lượng của các sản phẩm bán
	ArrayList<Integer> listQuantityProduct = CategoryDAO.getInstance().getQuantityProductSelling();
	Integer bestSelling=0;
	for (Integer value:listQuantityProduct){
		if (bestSelling<value){
			bestSelling=value;
		}
	}
	%>
	<h3 style="display: flex; justify-content: center; padding: 15px 0px 0px 0px;">Sản phẩm bán chạy nhất có số lượng là: <%= bestSelling %></h3>
	...
	var datas = [
		<% for (int i = 0; i < listQuantityProduct.size(); i++) { %>
        '<%= listQuantityProduct.get(i).toString() %>'<%= (i < listQuantityProduct.size() - 1) ? ',' : ' ' %>
    <% } %>
    ];
